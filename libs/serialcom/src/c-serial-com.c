...
//        if (0 == m_nTimeout) //unblock mode
        if (pthread_create(&sc->m_reader, NULL, ScThreadReader, sc) != 0) {
            TRACE0("Error on creating ScReader.\n");
            return false;
        }
        TRACE0("ScReader created successfully.\n");
        return true;
    } else {
        ScClose(sc);
        return false;
...
int ScRead(SerialCom* sc, const char* pData, int nSkip, const int nMax) {
    char* pOutput = (char*) pData + nSkip;
    int nMaxout = nMax - nSkip;
    
    if (pData == NULL) return -1;
    if (nSkip >= nMax) return 0;
